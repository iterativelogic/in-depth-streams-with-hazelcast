...
5. _In another window_, start a second container and use the REST API to verify that the members are in a cluster.
...
# Part 3 - Starting Jet Using Spring XML Context

Hazelcast provides a schema definition to facilitate incorporating a Jet instance into a Spring application.  In this lab we will look at defining and starting Jet using ClassPathXmlApplicationContext.  

Note: the schema files have been included with the project.  IntelliJ will automatically pick them up. Eclipse may require some manual configuration.  It is worthwhile getting your IDE set up to use these because of the auto-completion features.

1. Using [the documentation](https://docs.hazelcast.org/docs/jet/3.2/manual/#spring-integration), edit `config/jet-spring-application.xml` to match the configuration in hazelcast.xml.  Specifically configure the following:
   - Configure the join configuration of the underlying Hazelcast instance with multicast disabled.
   - Configure the tcp-ip join mechanism with one server: `jet-server-1:5701`
   - Enable the REST API and all of the endpoint groups.
   - Set the property `hazelcast.initial.min.cluster.size` to 2.
   - Specify the management center URL as `http://imdg-management-center:8080/hazelcast-mancenter`
2. 
