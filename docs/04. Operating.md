...
1. Copy `lab04.yml` to `docker-compose.yml`
...
3. Now, configure hot restart persistence. This configuration must be done in 2 places: the Jet configuration file and the IMDG configuration file.  Note that in `docker-compose.yml` we are using "-Dhazelcast.jet.config" to explicitly point Jet to the configuration file at `/opt/project/config/hazelcast-jet.xml`.
...
# Part 4: Update a Deployed Job Without Missing Events
...
2. Now  update the code. For this change, all you need to do is add the "obd_codes" field to the Ping java object at `ingest-pipeline/src/main/java/com/hazelcast/training/streams/ingest/Ping.java`.   Once you've updated the Ping object, rebuilt the ingest-pipeline jar.
...
Add another jet server to `docker-compose.yml`.  Be sure you change the service name and the container name to "jet-server-3".  In one window, watch the logs (`docker-compose logs --follow`), in another window, bring up the 3rd member with (`docker-compose up -d`).  
